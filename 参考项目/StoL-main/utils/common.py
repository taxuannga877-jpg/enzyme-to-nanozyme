...
def get_main_optimizer(cfg, model):
    if cfg.type == "adam":
        scaler_mlp_params = set(model.scaler_mlp.parameters())
        main_params = filter(lambda p: p not in scaler_mlp_params, model.parameters())
...
def get_scaler_optimizer(cfg, model):
    if cfg.type == "adam":
        return torch.optim.Adam(
            model.scaler_mlp.parameters(),
            lr=cfg.lr,
            weight_decay=cfg.weight_decay,
            betas=(
                cfg.beta1,
