...
def query_ec_from_uniprot(
    ...
    """
    从缓存文件或UniProt查询EC号的所有条目（包含AlphaFold ID）
    ...
    if cache_file.exists():
        try:
            ...
            if result:
                print(f"从缓存读取: {len(result)} 个有AlphaFold结构的条目")
                return result
        except Exception as e:
    ...
    print(f"从UniProt查询: {len(result)} 个有AlphaFold结构的条目")
    return result
...
def download_pdb_library(
    ...
    for i, ec_number in enumerate(ec_numbers, 1):
        ...
        entries = query_ec_from_uniprot(fetcher, ec_number, cache_dir, FETCH_ALL_RESULTS)
    ...
    for ec_number, entries in all_entries.items():
        ...
        for i, entry in enumerate(entries, 1):
            ...
            print(f"  [{i}/{len(entries)}] {alphafold_id} ({uniprot_id})...", end=' ', flush=True)
            
            # 下载到EC号子目录
            pdb_path = download_pdb_to_library(alphafold_id, ec_dir, timeout=30)
