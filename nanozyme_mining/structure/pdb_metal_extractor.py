...
class MetalSite:
    ...
    functional_role: str = "unknown"  # "catalytic" / "structural" / "substrate_binding" / "unknown"
...
class PDBMetalExtractor:
    ...
    def _is_metal_element(self, element: str) -> bool:
        ...
        return element_upper in KNOWN_METAL_ELEMENTS
    ...
    def parse_pdb_file(self, pdb_file: Path) -> List[MetalSite]:
        ...
        with open(pdb_file, "r", encoding="utf-8", errors="replace") as f:
            return self.parse_pdb_lines(f.readlines())

    def parse_pdb_lines(self, lines: List[str]) -> List[MetalSite]:
        """
        Extract metal binding sites from already-loaded PDB lines.

        This keeps parse_pdb_file backward compatible while allowing callers
        that need multiple PDB parsers to read the file only once.
        """
        # Parse PDB file
        atoms = self._parse_atoms_from_lines(lines)
        ...
        for metal in metal_atoms:
            ...
            if coordinating:  # Only include if there are coordinating residues
                ...
                site = MetalSite(
                    ...
                    functional_role=self._classify_metal_role(metal_type, coordinating)
                )
    ...
    def _parse_atoms(self, pdb_file: Path) -> List[Dict]:
        ...
        with open(pdb_file, "r", encoding="utf-8", errors="replace") as f:
            return self._parse_atoms_from_lines(f)

    def _parse_atoms_from_lines(self, lines) -> List[Dict]:
        """Parse ATOM and HETATM records from already-loaded PDB lines."""
        atoms = []
    ...
    def _classify_metal_role(self, metal_type: str, coordinating_residues: List[Dict]) -> str:
        res_names = [r.get("residue_name", "") for r in coordinating_residues]
        res_set = set(res_names)
        if metal_type in {"CA", "NA", "K", "MG"}:
            return "structural"
        if metal_type in {"ZN", "ZN2"}:
            if res_names.count("CYS") >= 3 and not (res_set & {"ASP", "GLU"}):
                return "structural"
        if metal_type in {"FE", "FE2", "FE3", "CU", "CU1", "CU2", "MN", "MN2", "CO", "CO2", "NI", "NI2"}:
            return "catalytic"
        if "HIS" in res_set and res_set & {"ASP", "GLU"}:
            return "catalytic"
        return "unknown"
